#!/usr/bin/env python
"""MXEで計算した温度輸送量を1つのnetCDFファイルにまとめる

Usage: make_trans_temp_netcdf.py
...
time=np.append(daj1.time.values,pd.date_range(start='2020-01-01',freq='D',periods=300))
time=np.append(time,dam1.time.values)

dummy_data=np.zeros(300)
dummy_data[:] = np.nan
...
v3=np.append(v3,dam3.values) * 1.e-6
da3 = xr.DataArray(data=v3,dims=('time'),attrs={'units':'C m3/s'})

v4=np.append(daj4.values,dummy_data)
v4=np.append(v4,dam4.values) * 1.e-6
da4 = xr.DataArray(data=v4,dims=('time'),attrs={'units':'C m3/s'})
