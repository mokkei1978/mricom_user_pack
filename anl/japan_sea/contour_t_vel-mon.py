#!/usr/bin/env python
"""水温分布と流速ベクトルを重ねて描く: 月毎

Usage: contour_t_vel.py YM
...
DS = DSu.assign(vo=DSv['vo']).isel(lev=20).squeeze()
da = DSt['thetao'].isel(lev=20).squeeze()
...
ax.quiver(x=DS['lon'].values[::thin],y=DS['lat'].values[::thin],
          ...
          color='black',transform=proj, scale=400)

ax.plot([127.,138.],[40.,40.],color='red')
ax.plot([138.,138.],[40.,48.],color='red')
