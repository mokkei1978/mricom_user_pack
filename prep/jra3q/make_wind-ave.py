...
def write_ctl(file_ctl, file_gd, lon, lat, time0, name, long_name):
    """grads ctl ファイルを書き出す(年平均1レコード、1変数)"""
    ...
    with open(file_ctl, 'w') as f:
        ...
        f.write('TITLE JRA-3Q annual mean wind stress\n')
        ...
        f.write(f'TDEF     1 LINEAR {grads_date(time0)} 1YR\n')
        f.write('VARS 1\n')
...
def main():
    ...
    varinfo = [
        (f'{base_out}x.gd', f'{base_out}x.ctl', 'uflx', ds['UFLX_surface'].mean(dim='time'),
         ds['UFLX_surface'].attrs.get('long_name', 'UFLX_surface')),
        (f'{base_out}y.gd', f'{base_out}y.ctl', 'vflx', ds['VFLX_surface'].mean(dim='time'),
    ...
    for file_gd, file_ctl, name, da, long_name in varinfo:
        ...
        with open(file_gd, 'wb') as f:
            dat = da.values.astype(np.float32)
        ...
        write_ctl(file_ctl, file_gd, lon, lat, time[0], name, long_name)
